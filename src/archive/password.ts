...
const RANDOM_BUFFER_MIN_LENGTH = 32;
...
/** Validates the user-entered password pair without persisting either value. */
...
/** Loads the optional convenience value from caller-provided same-origin storage. */
...
/** Stores or removes the convenience value; localStorage is not a secure vault. */
...
/** Removes the remembered convenience value from storage. */
export function clearRememberedPassword(storage: Storage): void {
    storage.removeItem(REMEMBERED_PASSWORD_KEY);
}

/** Generates a cryptographically random password from the documented alphabet. */
export function generateStrongPassword(
    ...
    const randomBytes = new Uint8Array(
        Math.max(RANDOM_BUFFER_MIN_LENGTH, Math.ceil((length * 256) / largestAcceptedByte)),
    );
